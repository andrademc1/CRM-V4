...
async function inicializarBancoDados() {
  ...
  try {
    ...
    // Criar tabela bookmaker_deals se não existir
    await client.query(`
      CREATE TABLE IF NOT EXISTS bookmaker_deals (
        id SERIAL PRIMARY KEY,
...
app.post('/bookmakers/salvar-deal', requireLogin, async (req, res) => {
  try {
    ...
    console.log('Dados recebidos para salvamento de deal:', {
      bookmaker_id, 
      deal_type, 
      deal_values,
      default_deal,
    ...
    if (!bookmaker_id || !deal_type) {
      return res.status(400).json({ 
        success: false, 
        error: 'Dados incompletos. Bookmaker ID e tipo de deal são obrigatórios.' 
      });
    }
    ...
    if (dealCheck.rows.length > 0) {
      ...
      console.log(`Atualizando deal existente ID: ${dealId}`);
      await client.query(
      ...
      // Criar novo deal
      console.log(`Criando novo deal para bookmaker ID: ${bookmaker_id}`);
      const result = await client.query(
...
// Rota para obter deals de um bookmaker
app.get('/bookmakers/get-deals/:id', requireLogin, async (req, res) => {
  const bookmakerId = req.params.id;
  ...
  try {
    ...
    const result = await client.query(
      'SELECT * FROM bookmaker_deals WHERE bookmaker_id = $1',
      [bookmakerId]
    );
    ...
    console.error('Erro ao obter deals do bookmaker:', err);
    res.status(500).json({ 
      success: false, 
      error: 'Erro ao obter deals: ' + err.message 
    });
  }
});
